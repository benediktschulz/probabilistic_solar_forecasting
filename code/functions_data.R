...
prep_na <- function(df, var_omit = NULL, var_replace = NULL){
  ...
  #..............Default: NULL -> None of the variables
  #  # n_roll........Number of days in moving average over day of year (integer)
  #  # ..............Default: 20
  ###-----------------------------------------------------------------------------
  ###Output
...
prep_data <- function(df, loc_path = NULL, pred_ens = c("ens_mean", "ens_sd"), 
  ...
  # Model surface height (at nearest grid point) of COSMO-D2
  if(is.element("orog_D2", pred_loc)){ df[["orog_D2"]] <- sapply(df[["location"]], function(x_loc){ 
    loc_data[["orog_D2"]][which(loc_data[["station_id"]] == x_loc)] }) }
  ...
  # Difference of station altitude and model surface height (at nearest grid point) of COSMO-D2
  if(is.element("orog_diff_D2", pred_loc)){ df[["orog_diff_D2"]] <- sapply(df[["location"]], function(x_loc){ 
    loc_data[["orog_D2"]][which(loc_data[["station_id"]] == x_loc)] - 
      loc_data[["height"]][which(loc_data[["station_id"]] == x_loc)] }) }
